...
module octave( input clk_in, input rst_in,
    ...
    //A4
    sine_generator  #(.PHASE_INCR(32'd39370534)) toneA(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_A4));  
    //A#4
    sine_generator  #(.PHASE_INCR(32'd41711627)) toneAs(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_As4));  
    //B4
    sine_generator  #(.PHASE_INCR(32'd44191930)) toneB(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_B4));
    //C5
    sine_generator  #(.PHASE_INCR(32'd46819719)) toneC(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_C5));
    //C#5
    sine_generator  #(.PHASE_INCR(32'd49603764)) toneCs(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_Cs5));
    //D5
    sine_generator  #(.PHASE_INCR(32'd52553357)) toneD(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_D5));
    //D#5
    sine_generator  #(.PHASE_INCR(32'd55678342)) toneDs(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_Ds5));
    //E5
    sine_generator  #(.PHASE_INCR(32'd58989149)) toneE(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_E5));
    //F5
    sine_generator  #(.PHASE_INCR(32'd62496826)) toneF(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_F5));
    //F#5
    sine_generator  #(.PHASE_INCR(32'd66213081)) toneFs(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_Fs5));
    //G5
    sine_generator  #(.PHASE_INCR(32'd70150316)) toneG(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_G5));
    //G#5
    sine_generator  #(.PHASE_INCR(32'd74321671)) toneGs(.clk_in(clk_in), .rst_in(rst_in), 
                               .step_in(step_in), .amp_out(tone_Gs5));
   
    logic [3:0] note_count;
    // TODO: need to actuall take log 2 of this...need module for that probably
    assign note_count = notes[11] +
        notes[10] + 
        notes[9] +
        notes[8] +
        notes[7] +
    ...
    assign amp_out = 
    notes[11] * (tone_A4>>note_count) + 
    notes[10] * (tone_As4>>note_count) + 
    notes[9] * (tone_B4>>note_count) + 
    notes[8] * (tone_C5>>note_count) + 
    notes[7] * (tone_Cs5>>note_count) + 
    notes[6] * (tone_D5>>note_count) + 
    notes[5] * (tone_Ds5>>note_count) + 
    notes[4] * (tone_E5>>note_count) + 
    notes[3] * (tone_F5>>note_count) + 
    notes[2] * (tone_Fs5>>note_count) + 
    notes[1] * (tone_G5>>note_count) + 
    notes[0] * (tone_Gs5>>note_count); 
