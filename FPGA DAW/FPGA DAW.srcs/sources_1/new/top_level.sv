...
//    logic [87:0] notes;
    ...
    logic ch;
    seven_seg_controller seven_seg_mod (.clk_in(clk_65mhz), .rst_in(btnc), .val_in({latest_valid_beat[0], metronome_raw_audio, 7'b0, metronome_main, beat_count}), .cat_out({ch,cg,cf,ce,cd,cc,cb,ca}), .an_out(an));
//    effects effects_mod ();
